...
| Adobe Target | Med möjligheten för AEM att ansluta till Adobe Target-tjänsten med det Adobe I/O-baserade API:t för Adobe Target Standard (Rest API) i AEM 6.5 är XML-metoden (Target Classic API) föråldrad. | Konfigurera om integreringen för att [använda det nya API:t](https://helpx.adobe.com/experience-manager/kt/sites/using/aem-sites-target-standard-technical-video-understand.html). |
...
| Forms | Uppgradering från LiveCycle ES4 SP1 till AEM 6.5 Forms på JEE är inte tillgänglig | Se [tillgängliga uppgraderingsalternativ](../forms/using/upgrade.md) i uppgraderingsdokumentationen för AEM Forms. |
