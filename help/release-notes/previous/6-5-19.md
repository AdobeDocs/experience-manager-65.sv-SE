...
* När du har uppgraderat en instans från AEM 6.5.17 och AEM 6.5.18, från sidredigeraren, omdirigeras du till en URL som inte finns om du valde **[!UICONTROL Publish Page]**. Användaren bör omdirigeras till Publish-guiden. (SITES-15856)
* Överflödig kopiering från AEM Urklipp vid en inklistring från operativsystemets Urklipp. (SITES-15704)
...
* När en resurs har överförts till AEM aktiveras arbetsflödet `update_asset`. Arbetsflödet avslutas aldrig. Om du tittar på arbetsflödesinstanserna avslutas arbetsflödet upp till produktöverföringssteget. Nästa steg är batchöverföring från Scene7. Användaren kan se att resursen finns i Scene7 från Dynamic Media Classic-appen. (ASSETS-30443)
...
* Även efter att Synkronisering har inaktiverats på mappnivå visar loggarna utlösaren Scene7 ReplicateOnModifyListener. `ReplicateOnModifyListener/Worker` bör hoppa över bearbetning av icke-Dynamic Media mappresurser och innehållsfragment. (ASSETS-26705)
...
Det finns två olika metoder som du kan använda för att automatiskt installera [!DNL Experience Manager] 6.5.19.0.<!-- UPDATE FOR EACH NEW RELEASE -->
...
UberJar för [!DNL Experience Manager] 6.5.19.0 är tillgängligt i [Maven Central-databasen](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.19/). <!-- CHECK FOR UPDATE EACH NEW RELEASE -->
...
* Om du uppgraderar till AEM Service Pack 6.5.15.0 eller senare, och din AEM körs på Tomcat 8.5.88, måste du installera servletsfragmentet *innan* du fortsätter med installationen av Service Pack 6.5.15.0 eller senare.
...
* När du har installerat AEM Service Pack 6.5.14.0 och senare kan användare inte välja ett teckensnitt i JEE Admin-gränssnittet för PDF-dokument när de navigerar till `Home` > `Services` > `PDF Generator` > `Adobe PDF Settings`, eftersom teckensnittslistan är tom. (FORMS-12095)
...
* [Lista över OSGi-paket i Experience Manager 6.5.19.0](/help/release-notes/assets/65190_bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
* [Lista över innehållspaket i Experience Manager 6.5.19.0](/help/release-notes/assets/65190_packages.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
