...
[!DNL Adobe Experience Manager] 6.5.11.0 innehåller nya funktioner, viktiga förbättringar som kunderna efterfrågat samt förbättringar av prestanda, stabilitet och säkerhet som släppts sedan 6.5-versionen släpptes i april 2019. Service Pack har installerats på [!DNL Adobe Experience Manager] 6.5.
...
* Du kan nu använda [!DNL Dynamic Media] för att konfigurera allmänna inställningar i stället för att behöva gå igenom skrivbordsprogrammet [!DNL Dynamic Media Classic]. Se [Konfigurera allmänna inställningar för dynamiska media](/help/assets/dm-general-settings.md).

  ![allmänna DM-inställningar](/help/assets/assets-dm/dm-general-settings.png)

* Du kan nu använda [!DNL Dynamic Media] för att konfigurera publiceringsinställningar i stället för att behöva gå igenom skrivbordsprogrammet [!DNL Dynamic Media Classic]. Se [Konfigurera inställningar för dynamisk mediepublicering](/help/assets/dm-publish-settings.md).
...
* I en anpassad dialogruta som används i ett arbetsflöde, misslyckas Experience Manager när dialogrutan valideras, med ett fel i webbläsarkonsolen (GRANITE-35049).
...
* Felaktig PDF-återgivning genereras för broschyr-, flyer- och InDesign-mallarna (NPR-36433).
...
* Om [!DNL Assets] är inaktiverat i [!DNL Launcher] fungerar inte återkoppling av metadata när den aktiveras manuellt (CQ-4329082).
...
* Problem vid bearbetning av PDF-resurser om det finns stora databaser (CQ-4328711).

* PDF-bearbetningsfel sprids inte till [!DNL Experience Manager] om det uppstår ett fel vid [!DNL Scene7] (CQ-4331145).
...
* I [!DNL Experience Manager]-uppsättningar som innehåller anpassad startfunktion med `ActivationModel` som arbetsflöde kraschar Experience Manager på grund av minnesproblem vid överföring av PDF-filer. (CQ-4330512)
...
* När du uppgraderar från [!DNL Experience Manager] 6.5.8.0 till version 6.5.9.0 skriver installationen över egenskaperna för `/content/dam/projects`. Det återställer mappens tilldelade metadatamatchema och egenskaper till standard (NPR-37124).
...
* När du installerar AEM 6.5.10.0 fungerar inte HTML-förhandsgranskningen för ett XDP-formulär (NPR-37503, CQ-4331926).

* När användare försöker konvertera XDP-formulär från PDF till HTML5-formulär. Följande problem kan uppstå vid återgivning av HTML5-formulär för kambodjanska teckensnitt:
...
* När du skickar ett brev och öppnar det igen i HTML-vyn ändras inte textdokumentfragmentens placering (NPR-37307).
...
**Problem som har åtgärdats i AEM Forms6.5.11.1**

>[!NOTE]
>
>Om du inte har uppgraderat till AEM 6.5.11.0 Forms installerar du AEM Forms 6.5.11.1-tilläggspaketet direkt. Om du har installerat AEM 6.5.11.0 Forms bör du uppgradera till AEM 6.5.11.1 Forms.

* Skicka åtgärder, Skicka e-post och Anropa ett AEM-arbetsflöde fungerar inte när Forms 6.5.11.0-tilläggspaketet har installerats.
* CreatePDF-åtgärden avbryter konverteringen av Microsoft Word-dokument till PDF-dokument när Forms 6.5.11.0-tilläggspaketet har installerats.
* (Endast JEE) Allvarliga säkerhetsluckor (CVE-2021-44228 och CVE-2021-45046) rapporterades för Apache Log4j2.
* (Endast JEE) Assembler DSC i patchen 6.5.11.0 innehåller felaktiga metainfo som specifikationsversion och impl-version.
* När användare försöker konvertera XDP-formulär från PDF till HTML5 är några av de åtgärdade problemen:
   * Stöd för persiska som ska återges som LTR.
   * Stöd för RTL vid blandning med LTR-text.
   * Stöd för bildrotation.
   * Stöd för det kambodjanska språket läggs till. Om du vill aktivera stöd för Combodian måste du aktivera flaggan som `CTL=true` när du återger HTML5-formulär.
...
* Installera Experience Manager 6.5.11.0 på en av författarinstanserna med Package Manager på en distribution med MongoDB och flera instanser.
...
UberJar för Experience Manager 6.5.11.0 är tillgänglig i [Maven Central-databasen](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.11/).
...
* Om du använder innehållsfragment och GraphQL rekommenderar vi att du uppgraderar till 6.5.12.0 och installerar rätt paket.
   * Om det inte går att uppgradera till 6.5.12.0 kontaktar du supporten för att utvärdera om en tillfällig lösning kan implementeras under tiden.
...
* [Lista över OSGi-paket som ingår i Experience Manager 6.5.11.0](../assets/65110_bundles.txt)
* [Lista över innehållspaket som ingår i Experience Manager 6.5.11.0](../assets/65110_packages.txt)
