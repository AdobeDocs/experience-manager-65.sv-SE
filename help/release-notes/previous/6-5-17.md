...
* När du publicerar mer än 40 PDF samtidigt slutar [!DNL Experience Manager] svara och blir otillgänglig under en tid. (ASSETS-21789)
...
* När du har uppdaterat till AEM 6.5.15.0 Service Pack fungerar inte HTML5-formulären och läses in korrekt i Edge webbläsare med kompatibilitetsläget IE. (FORMS-8526, FORMS-8523)
...
* När en användare uppgraderar till Experience Manager 6.5.16.0 Service Pack saknas CRX-metadataegenskaper efter att de skickats in. (FORMS-8205)
...
* PAC 2021-verktyget för alla bakgrundsobjekt i ett PDF-formulär som har skapats av Experience Manager Designer returnerar ett fel som `Path object not tagged`. (FORMS-6707)
...
* Om användaren skapar ett statiskt PDF med hjälp av utdatatjänsten i en Experience Manager Forms Designer 6.5 inträffar ett fel som `OCCD (optional content configuration dictionary) contains AS key`. (FORMS-6691)
...
* När en användare försöker generera ett PDF med hjälp av utdatatjänsten, trots att den är markerad som `PDF/A-1a`, misslyckas en kompatibilitetskontroll med hjälp av tjänsten `Preflight`. (LC-3920837)
...
* När en användare uppdaterar till Service Pack för Experience Manager 6.5.16.0 gäller inte `Usage Rights`. (FORMS-7892)
...
* När en användare försöker anropa en LCA-process med ett PDF som omfattar `Chinese Full Width Characters` uppstår ett problem med `ValidateForm`-processen. (FORMS-7464)
...
* Lägger till `modifyProperties`-behörighet för `fd-cloudservice`-systemanvändare för Adobe Sign-konfiguration. (FORMS-6164)
...
* Sling `ResourceMerger` förbrukar en hög mängd CPU när den tillhandahålls med en fiktiv sökväg, vilket orsakar ett denial of service-fel. (NPR-40338)
...
Det finns två olika metoder som du kan använda för att automatiskt installera [!DNL Experience Manager] 6.5.17.0.<!-- UPDATE FOR EACH NEW RELEASE -->
...
UberJar för [!DNL Experience Manager] 6.5.17.0 är tillgängligt i [Maven Central-databasen](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.17/). <!-- CHECK FOR UPDATE EACH NEW RELEASE -->
...
* På JBoss® 7.1.4-plattformen misslyckas distributionen av `adobe-livecycle-jboss.ear` när användaren installerar Service Pack för Experience Manager 6.5.16.0 eller senare.
...
* [Lista över OSGi-paket i Experience Manager 6.5.17.0](/help/release-notes/assets/65170_bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
* [Lista över innehållspaket i Experience Manager 6.5.17.0](/help/release-notes/assets/65170_packages.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
