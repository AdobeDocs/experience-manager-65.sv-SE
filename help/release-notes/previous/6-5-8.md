...
[!DNL Adobe Experience Manager] 6.5.8.0 innehåller nya funktioner, viktiga förbättringar som kunderna efterfrågat samt förbättringar av prestanda, stabilitet och säkerhet som släppts sedan 6.5-versionen släpptes i april 2019. Service Pack har installerats på [!DNL Adobe Experience Manager] 6.5.
...
* När du startar en AEM-förekomstuppgradering från AEM 6.3 till AEM 6.5 tar det längre tid att starta uppgraderingsförekomsten (NPR-35323).
* När du replikerar en AEM-resurs som innehåller en hakparentes (). i namnet misslyckas replikeringen (GRANITE-27004, NPR-35315).
...
* När flera sidor samtidigt kommer åt ett klientbibliotek kan HTML bibliotekshanterare inte läsa in klientbiblioteket. Det leder till felaktig återgivning av sidor (NPR-35538).
...
[!DNL Adobe Experience Manager] 6.5.8.0 [!DNL Assets] åtgärdar följande problem och erbjuder följande förbättringar.
...
* [!DNL Assets] visar inte RGB-återgivning för en resurs som har RGB-återgivning (CQ-4310190).
...
* Fliken [!UICONTROL To-do] på arbetsytan i HTML visar HTML-tecken (NPR-35298).
...
* Den PDF forms som genereras med Output Service innehåller inga kantlinjer (CQ-4312564).
...
* Om du uppgraderar din [!DNL Experience Manager]-instans från 6.5 till 6.5.8.0 kan du visa `RRD4JReporter` undantag i `error.log`-filen. Starta om instansen för att lösa problemet.
