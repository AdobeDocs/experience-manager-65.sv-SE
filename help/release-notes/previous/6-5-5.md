...
![Anpassa Experience Manager Inbox-kolumner](/help/release-notes/assets/customize-columns.gif)

* Förbättrad tillgänglighet inom olika områden av Experience Manager Web Content Management (WCM), t.ex. sidredigeraren, kärnkomponenter, RTE och administratörsgränssnittet.
...
* För att hämta publicerings-URL för Dynamic Media Scene7 har en ny metod, `getRemoteAssetPublishURL`, lagts till i `com.day.cq.dam.api.s7dam.scene7.ImageUrlApi`-gränssnittet.
...
### Tillgänglighetsförbättringar (6.5.5.0) {#accessibility-sites}
...
* Under XMP tillbakaskrivningsprocess ändrar alla anpassade namnutrymmesmetadataegenskaper det anpassade namnområdesprefixet till ns2 i motsats till det namnområdesprefix som har konfigurerats (NPR-32748).
...
* Resursnamn ändras till gemener när resurser flyttas från en mapp till en annan i [!DNL Adobe Experience Manager] som arbetar i körläget Dynamic Media Scene7 (NPR-32995).
...
* Bildförinställningar publiceras inte om [!UICONTROL Dynamic Media sync mode] är inaktiverat som standard på Experience Manager med körningsläget Dynamic Media Scene7 (CQ-4294200).
...
* Prestandaproblem som har korrigerats i 6.5.5.0 är (CQ-4279206):

   * Det tar för lång tid att överföra stora binära filer till servrar för bildbearbetning i dynamiska media.

   * Tiden för generering av miniatyrbilder i Experience Manager ökar tack vare Dynamic Media Scene7-arkitekturen.
...
* SSRF-säkerhetslucka i Experience Manager (NPR-33435).
...
* Kärnpaketet [!DNL Jackson] saknas i Experience Manager uberjar-filen (NPR-32848).
* CRXDE Lite läser inte in innehåll för användare utan läsbehörighet för egenskapen `jcr:primaryType` för en nod (NPR-32611).
* [!DNL Granite]-underhållsschemaläggaren initierar om för ofta under Experience Manager-distributioner (CQ-4294627).
...
* När du öppnar ett meddelande i Experience Manager Inbox ändras inte meddelandets status till Läs (NPR-32764).
...
* Adaptiv Forms-tillgänglighet: När en användare förhandsgranskar ett adaptivt formulär som ett HTML-formulär kan fältet [!UICONTROL Scribble Signature] inte behålla tabbfokus (NPR-33159).
...
* PDFG-tjänst: När en användare konverterar en textfil till en PDF återges inte japanska tecken korrekt (NPR-33238).
* PDFG-tjänst: `CreatePDF`-åtgärden kan inte konvertera en PDF-fil till PDF OCR-format (NPR-32994).
* PDFG-tjänst: PDF-konvertering misslyckas för den 200:e instansen av ett [!DNL OpenOffice]-dokument (NPR-32766).
...
* Designer: Om taggningsalternativet är aktiverat försvinner delformulärsramen i de genererade PDF-utdata (NPR-32778).
...
UberJar för Experience Manager 6.5.5.0 är tillgänglig i [Maven Central-databasen](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.5/).
