...
* Du kan nu synkronisera selektiva innehållsunderträd till *Dynamiska media - Scene7-läge* i stället för alla tillgängliga på `content/dam`.

* Integrering av formulärdatamodeller med SOAP webbtjänst har nu stöd för urvalsgrupper eller attribut för element.

* SOAP in- och utdata och komplexa datastrukturer har nu stöd för dynamisk gruppersättning.
...
* När en URL för en Adobe Experience Manager Sites-sida innehåller ett kolon (`:`) eller en procentsymbol (`%`) slutar webbläsaren att svara och CPU använder toppar (NPR-32369, NPR-31918).
...
* När du väljer alternativet Dra komponent här i platshållaren för ett responsivt rutnät skickas en GET-begäran och begäran resulterar i HTTP 403-fel (NPR-31845).
...
* Det går inte att synkronisera kampanjen i Adobe-kampanjen som innehåller en färdig designimportkomponent (NPR-31728).
...
* Assets detaljsida för PDF-resurser visar inte åtgärdsknappar förutom knapparna för samling och Lägg till återgivning i Experience Manager som körs i körläget Dynamic Media Scene7 (CQ-4286705).

* Assets tar för lång tid på sig att bearbeta genom batchöverföringen av Scene7 (CQ-4286445).
...
* Carousel Editor öppnas inte i IE 11 i Hybrid-läget för Experience Manager Dynamic Media (CQ-425590). **För kunder med Adobe Dynamic Media - hybrid-läge:** Adobe upphör stödet för Internet Explorer 11 i läget Dynamic Media - hybrid efter maj 2022.
...
* Designer: Om taggningsalternativet är aktiverat försvinner delformulärsramen i de genererade PDF-utdata (NPR-32547, NPR-31983, NPR-31950).

* Designer: Om det finns sammanfogade celler i en tabell misslyckas hjälpmedelstestet för utdata-PDF-filen som konverterats från ett XDP-formulär med hjälp av utdatatjänsten (CQ-4285372).
...
UberJar för Experience Manager 6.5.4.0 är tillgänglig i [Maven Central-databasen](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.4/).
