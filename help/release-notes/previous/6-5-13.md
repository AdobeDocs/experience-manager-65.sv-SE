...
[!DNL Experience Manager] 6.5.13.0 innehåller nya funktioner, viktiga förbättringar som kunderna efterfrågat samt förbättringar av prestanda, stabilitet och säkerhet som släppts sedan den första tillgängligheten av 6.5 i april 2019. [Installera detta Service Pack](#install) på [!DNL Experience Manager] 6.5.
...
* Tillgänglighetsförbättringar för Forms:
   ...
   * Stöd för att skärmläsare ska sluta navigera på HTML arbetsyta tills en dialogruta är öppen har lagts till. (NPR-37134)

Följande felkorrigeringar, nyckelfunktioner och förbättringar introducerades i [!DNL Experience Manager] 6.5.13.0:
...
* Tokenbaserad säker förhandsgranskning för Dynamic Media-resurser på AEM-författare. (ASSETS-4995)
* När du skapar en bildförinställning för dynamiska media i [!DNL Experience Manager] är den tillåtna maxstorleken begränsad till 2 000 × 2 000 pixlar i användargränssnittet. När värdet ökas till 2 001 pixlar för antingen bredd eller höjd inaktiveras knappen **[!UICONTROL Save]**. (ASSETS-5691)
...
* När du har lagt till en Dynamic Media-komponent på en webbplats och efter att du har publicerat sidan visas inte den nya Dynamic Media-resursen på den publicerade sidan och den kan inte heller visas på förhandsgranskningssidan. Problemet uppstod för både bild- och videoresurstyper. (ASSETS-9467)
...
* Den PDF417-streckkodskodning som skapas av [!DNL Experience Manager] Forms är ogiltig för en alternativknappsgrupp. (NPR-38525)
...
* Användare loggas ut slumpmässigt från [!DNL Experience Manager] publiceringsinstanser och ett adaptivt formulär kan inte skickas. Problemet visas på [!DNL Experience Manager] instanser som använder Dispatcher. (NPR-38285)
* När du använder alternativet getFormDataString i en regel i Adobe Launch för att hämta adaptiva formulärdata, returnerar alternativet inte adaptiva Forms-data. (NPR-38283)
...
* När en AEM Forms JEE-tjänst anropas med SOAP slutpunktstjänstmetod på en knapp som konfigurerats som en regel för adaptiv form misslyckas SOAP-tjänsten med följande undantag:
...
* Om du använder com.adobe.fd.pdfutility.services.PDFUtilityService#convertPDFtoXDP för att konvertera en PDF till XDP-format returneras en ogiltig XDP-fil. (NPR-38140, CQ-4342099)
...
* AEM Forms-komponenten som är inbäddad på SITES-sidan använder breddattributet som har värdet % och som inte är giltigt enligt W3C HTML-validering. Användarna råkade ut för ett felaktigt tolkningsfel under HTML-valideringen. (NPR-38124)
...
* När en användare lägger till HTML-taggar i kommentarsavsnittet när ett arbetsflöde körs återges HTML-taggarna. (NPR-37591)
...
* Det går inte att konvertera en bild till PDF med PDFG-tjänsten med OCR efter att du har använt patchen AEMForms-6.5.0-0038 (log4jv2.16). (CQ-4342450)
...
* Oak-versionen för 6.5.13.0 har nu uppdaterats till 1.2.11. (NPR-38084)
...
* Från och med 6.5.13 behöver du inte längre skapa behållarkomponentnoden i JCR innan du redigerar via SPA-redigeraren. En `virtual container` skapas innan den sparas med SDK. (SITES-5762)
...
Det finns två olika metoder som du kan använda för att installera [!DNL Experience Manager] 6.5.13.0 automatiskt.
...
UberJar för [!DNL Experience Manager] 6.5.13.0 är tillgänglig i [Maven Central-databasen](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.13/)&#x200B;(https://).
...
* [Lista över OSGi-paket som ingår i Experience Manager 6.5.13.0](/help/release-notes/assets/65130_bundles.txt)

* [Lista över innehållspaket som ingår i Experience Manager 6.5.13.0](/help/release-notes/assets/65130_packages.txt)
