...
* Skapa ett varumärke under `/content/campaign` resulterar i en struktur som inte gör att ni kan skapa kampanjer. [!UICONTROL Create Brand] lämnar det nyskapade varumärket utan möjlighet att skapa [!UICONTROL Offers and Activities] eftersom det inte finns något [!UICONTROL Create] option (NPR-34113).
...
* När du navigerar i bläddringsläge i dialogrutan för länkdelning visas en skärmläsare,
   ...
   * Lägger till en berättarröst för de automatiska förslagen i [!UICONTROL Add Email Address/Search] (CQ-4294232)
...
* Användaren kan nu ändra fokus med tangentbordet i:
   ...
   * [!UICONTROL Search/Add Email Address] fältet i [!UICONTROL Link Sharing] -dialogrutan.
...
* Resursmappar publiceras inte med [!UICONTROL Manage Publication] om mapparna innehåller resurser i filformaten JavaScript, CSS eller JSON (NPR-34090).
...
* Om användaren flyttar en publicerad resurs till en ny plats publiceras resursen om, även om [!UICONTROL Republish] alternativet är avmarkerat. Detta leder till att många överblivna resurser ligger i publiceringsinstansen. Standardbeteendet är dock att om du flyttar en åtgärd för en publicerad resurs återpubliceras den automatiskt. Den här resursen publiceras igen om författaren väljer [!UICONTROL Republish] option when moving the asset (NPR-33934).
...
* JavaScript-fel observeras och användare kan inte markera eller ta bort de alternativ som genererats i [!UICONTROL Dropdown] fält efter [!UICONTROL Add through JSON path] i [!UICONTROL Folder Metadata Schema Form Editor], om den överförda JSON-filen har blanksteg eller specialtecken (NPR-33712).
...
* Körningskopian av resurser, anpassad arbetsflödesmodell (skapad i `/var/workflow/models/dam`) tas bort vid installationen [!DNL Experience Manager] 6.5 Service Pack 5 eller en tidigare version på [!DNL Experience Manager] 6.5 (NPR-34532). Om du vill hämta körtidskopian synkroniserar du designtidskopian av arbetsflödesmodellen med körtidskopian med HTTP API:
...
* Symboler [!UICONTROL Behavior] flik för [!UICONTROL Viewer Preset Editor] är inte lokaliserade (CQ-4299026).
...
* Tryck på `Esc` när du söker efter användare att personifiera, **[!UICONTROL User]** dialogrutan stängs i stället för användarlistan (NPR-34084).
...
* The **[!UICONTROL Submit to REST endpoint]** Skicka fungerar inte för anpassningsbara formulär (CQ-4299044).
