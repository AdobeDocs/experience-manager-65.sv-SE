...
* **Stöd för att skicka anpassad XCI:** Stöd har lagts till för att skicka anpassad XCI i parametrar för cmdline-programmet xmlformcmd. Med den här funktionen kan användarna ange anpassade XCI-filer för testning, vilket ger större flexibilitet och kontroll över testprocessen. (LC-3923248)
...
* Skärmläsare får ett `Loading`-meddelande som talas medan platsvyn uppdateras. Uppdateringen lägger till en dedikerad statusregion och skriver meddelandet i den programmatiskt, vilket bekräftar förloppet utan att flytta fokus. (SITES-24481)
...
* Alternativtextfältet i textredigerarens hyperlänksdialogruta visar nu en programmatisk etikett. Skärmläsare meddelar `Alternative text` om fältet och fokusområdena på den namngivna kontrollen. Den här korrigeringen förbättrar navigeringen för tangentbords- och talanvändare. (SITES-24675)
...
* Dialogrutan Bild meddelar nu inläsningsstatus via en ARIA-region. Skärmläsare hör ett `Loading, please wait`-meddelande när rotationsrutan visas. Och en klar uppdatering när innehållet är klart, så att användarna vet när de kan interagera. (SITES-24697)
* Dialogrutan Länkval visar nu en aktiv region med sökresultat. Skärmläsare lyssnar på statusen `results updated` efter varje sökning utan att flytta fokus, så användarna får en tydlig bekräftelse på att sökningen har slutförts. (SITES-24700)
...
* Ändra filter rapporterar nu status som expanderat eller komprimerat. Knappen växlar `aria-expanded` synkroniserat med filterpanelen och visar ett enda tydligt namn (`Change filters`), vilket tar bort det förvirrande `filter?`-meddelandet. Skärmläsaranvändare kan förutsäga resultatet av att aktivera kontrollen. (SITES-24713)
...
* Resultatstatusen visas nu för skärmläsare när filtren ändras. NVDA läser både antalet `X of Y results` och meddelandet `no results`. Sidindelningsstatusen använder en aktiv region som uppdateras på plats, så att användarna får en bekräftelse utan att flytta fokus. (SITES-24720)
...
* Rollen `presentation` som dolde tabellen för webbplatsens hemsida har tagits bort från hjälpmedelsträdet. Tabellen läses korrekt igen. NVDA och JAWS identifierar tabellen, känner igen rubriker och meddelar rubrikrelationer under rad- och kolumnnavigering. (SITES-24942)
...
* Statusuppdateringar i filtret Platsadministratörer meddelar skärmläsare. När användare byter kort/lista medan objekt läses in, talar NVDA nu `Please wait`-meddelandet via en aktiv region. Den här vägledningen förhindrar extra klick och förvirring. (SITES-24992)
...
* Åtgärden för att bekräfta i dialogrutan Teaser meddelar skärmläsare korrekt. Kontrollen rapporterar `Confirm` inte ikonbeskrivningen och ger användare av tangentbord och skärmläsare tydliga riktlinjer. (SITES-25223)
* Hjälpknappen visar nu ett tydligt namn. Skärmläsare meddelar `Help` i stället för en detaljerad ikonbeskrivning. Användarna förstår åtgärden och kan få hjälp snabbare. (SITES-25224)
...
* Skärmläsare meddelar nu kontrollerna Anteckna och Stäng anteckningar korrekt i verktygsfältet Anteckningar. NVDA säger inte längre `Preview button pressed,` som vilseleda författare och föreslog fel åtgärd. Meddelandet matchar knappen som trycktes ned och håller arbetsflödet tydligt. (SITES-25234)
...
* Skärmläsare meddelar de rätta åtgärderna för knapparna **Anteckning** och **Stäng anteckning**. NVDA säger inte längre `Preview button pressed`, det meddelar knappnamnet så att användarna vet när anteckningsläget startar eller avslutas. (SITES-25268)
...
* Knappen Redigera meddelar nu skärmläsare i det sekundära verktygsfältet rätt åtgärd. Aktiveringen läser `Edit` i stället för den orelaterade `Preview button pressed,` som tar bort förvirring under tangentbordsnavigering. (SITES-25320)
* Nu visas ett lämpligt namn i diagramreglaget för demografi. Skärmläsare meddelar `Cart total` och inmatningsverktyg för tal kan rikta kontrollen efter namn, vilket förbättrar kompatibiliteten med WCAG 4.1.2 (Namn, Roll, Värde). (SITES-25322)
...
* Med referensmarkeringen för järnväg visas nu namnet för skärmläsare. Regionen visar en unik `aria-label` (`References rail`) som förbättrar navigeringen för landmärken och skiljer den från andra regioner. (SITES-24973)
...
* Listrutan Kategorier saknade en specifik etikett, så JAWS talade om en allmän `images button menu.` Uppdateringen namnger kontrollen `Categories` och definierar dess roll. Skärmläsaranvändare hör en korrekt etikett och förstår vilka alternativ som är tillgängliga. (SITES-35226)
...
* Det multifält som används i en anpassad komponent som visas med ikonknappar utan etikett och med inkonsekvent tabbordning. JAWS/NVDA meddelade endast `button` eller överhoppade kontroller, vilket blockerade tangentbordsåtgärder. Uppdateringen innehåller beskrivande namn för Lägg till, Ta bort och Flytta, normaliserar tabbstopp och meddelar listuppdateringar för att uppfylla ADA:s förväntningar. (SITES-30660)
...
Parenteser i ett innehållets fragmentnamn orsakade att referenspanelen felaktigt rapporterade användningen. Författare såg 0 även när andra fragment refererade till det. Korrigeringen korrigerar sökvägsparsningen för `(` och `)` och använder rätt antal och poster som inte är noll. (SITES-35078)
...
* Experience Fragment-egenskaper översätter nu fliken **Referenser**. Etiketter och kolumnrubriker som `Page,` `Page path,` och `Variation title` visas på författarens språk. Den här ändringen tar bort strängar som bara är för engelska och gör egenskapsvyn konsekvent för globala team. (SITES-11203)
...
* Anpassade taggetiketter misslyckades i sidredigeraren och användargränssnittet visade alltid `Tags.` Predikatet utvärderar nu `fieldLabel` först och `labelText` sekund, och tillämpar sedan standardvärdet. Författare ser etiketten som de anger. (SITES-32278)
...
* Användare råkade ut för tillgänglighetsproblem i DoR PDF forms där NVDA felaktigt meddelade `unavailable` om kryssrutor, alternativknappar och textfält. Meddelandet upprepades ofta, vilket förvirrade användare av skärmläsare. (LC-3923201)
...
* Användare fick en tom primär spanska sida när endast engelskt innehåll valdes i ett XDP (XML Data Package) med flera primära sidor. (LC-3923009)
* Användare observerade föråldrad information om copyright-år i AEM Designer. Det här felet uppstod i popup-rutan vid start, i avsnittet `About` och i avsnittet `Legal Notices`, som visade `2003-2024` i stället för&quot;2003-2025&quot;. (LC-3923005)
* En tom PDF-sida påträffades när sidnumreringen användes i AEM Forms Designer. Problemet uppstod när `Top of the Next Page/Top of Page` valdes för WireAdviceHeader, vilket stör layouten för dataiterationer. (LC-3922997, LC-3922830)
* Ett problem uppstod där filsammandragsvärdet för XML-schemadefinitionen (XSD) inte kunde sparas i 64-bitarsversionen av AEM Forms Designer. (LC-3922924)
...
* Användare råkade ut för ett fel med bakgrundsfärgen för rubriken `Dati Richiedente` i PAC (Portable Application Component), som fick meddelandet `path object not tagged.` (LC-3922912)
...
* Användarna fick problem när de skapade regler för att visa en panel och sätta fokus på ett element i den. Regeln setFocus körs före synlighetsuppdateringen, vilket gör att fokusåtgärden misslyckas. (FORMS-19563)
...
* Ett `Invalid URL`-fel uppstod när användare försökte kommentera resurser i AEM 6.5.2. (NPR-42684)
...
* Hjälpikonen visar nu en kortfattad etikett för hjälpmedelstekniker. JAWS läser `Help button` och lägger inte längre till överflödiga `menu`-ord. Den här uppdateringen gör att kontrollen överensstämmer med WCAG 4.1.2 och förenklar användningen av tangentbord och skärmläsare. GRANITE-55360
...
* Hälsostatusaggregering har korrigerats i Systemöversikt. Vyn uppdateras nu när enskilda kontroller har körts och rätt antal visas. Operatörerna ser `OK` när säkerhets- och underhållskontroller godkänns, i stället för en felaktig `2 errors`-banderoll. GRANITE-61482
...
* Korrigerad trunkerad knappbeskrivningstext på panelen Bild. Beskrivningen `Display caption as pop-up` återges fullständigt på alla språkområden som stöds, vilket förbättrar vägledningen för författare som inte är engelska. (SITES-10490)
* Spaltvyn Webbplatsadministration trunkerade lokaliserade etiketter på franska och spanska. `End Time` och `Off Time` verkade vara trunkerade och visade inget verktygstips. Adobe korrigerade översättningarna och återställde verktygstipset vid hovring, så etiketterna lästes in fullständigt. (SITES-31318)
* Dialogrutan **Flytta** på platser visade i18n-nycklar i Raw-format i stället för läsbara etiketter. Objekt som `Referencing pages,` `Created on,` `Created by,` och `Path` ser förvrängda ut. Korrigeringen kopplar dialogrutan till rätt ordlistor och förser översättningarna med en engelsk reservtext. (SITES-30881)
...
* Korrigerade ett oväntat JSP-kompileringsfel med paketet `org.apache.sling.scripting.jsp:2.6.0`. (SLING-12442)
...
### Stöd för innehållsfragment i AEM Assets REST API {#cf-support-assets-rest-api}

AEM 6.5 LTS SP2 innehåller moderna OpenAPI:er för Content Fragment och Model Management, så de äldre slutpunkterna för Content Fragment Support i AEM Assets REST API är nu inaktuella.

Adobe har för avsikt att behålla dessa äldre endpoints tillgängliga tills ett meddelande om att de är färdigbehandlade. Adobe planerar inga ytterligare förbättringar för de borttagna slutpunkterna.
...
* **FORMS-23722** När ett formulär som innehåller ett **fält för bifogad fil** som använder `bindref` skickas till ett AEM-arbetsflöde med steget **Tilldela uppgift** visas inte de bifogade filerna. Därför visas de inte när uppgiften öppnas från Inkorgen. Filerna sparas korrekt i databasen, men användargränssnittet för Tilldela uppgift kan inte visa de bifogade filerna.
* **Anpassade funktioner i FORMS-23802** kan inte läsas in i förhandsvisning eller publicering när ett anpassat formulär är inbäddat på en webbplatssida. Problemet inträffar när biblioteksversionen **aem-forms-core-component** är tidigare än 1.1.76. Du kan se ett fel som `InvalidFormContainerException: No form container found` i loggarna. Du löser det här problemet genom att [hämta och installera snabbkorrigeringen](/help/release-notes/aem-forms-hotfix.md) för AEM Forms SP24 (AddOn 6.0.1454).

#### Kända problem med tillgängliga snabbkorrigeringar {#aem-forms-issues-with-hotfixes}
...
* **FORMS-23881** I AEM Forms JEE-distributioner som konfigurerats med det fullständiga installationsprogrammet 6.5.23.0 kan Output Service inte bearbeta begäranden när en anpassad XCI-fil anges i anropet. Du löser det här problemet genom att installera det senaste AEM 6.5.24.0 Forms Service Pack-paketet från [programdistributionsportalen](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html).

* **FORMS-23789** (endast AEM Forms på JEE): Användare fick problem med Log4j i AEM Forms på JEE SP24, vilket orsakade avbrott i loggning och övervakning för företagskunder. Du löser det här problemet genom att [hämta och installera snabbkorrigeringen](/help/release-notes/aem-forms-hotfix.md) för AEM Forms på JEE Service Pack 6.5.24.0.

* **FORMS-23802** Anpassade funktioner läses inte in i förhandsgranskning eller publicering när formuläret finns på en webbplatssida med en äldre version av aem-forms-core-component (&lt;1.1.76). Du löser det här problemet genom att installera [AEM Forms AddOn hotfix 6.0.1454](/help/release-notes/aem-forms-hotfix.md) för SP24.
...
* **FORMS-23979** HTML-till-PDF-konvertering (PDFG) kan få tillfälliga timeout. En nyare version av Forms-tillägget för SP24 släpptes sedan som innehåller korrigeringen. Om du råkar ut för det här problemet ska du uppdatera din miljö till den [senaste släppta Forms-tillägget för 6.5.24.0](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases#aem-65-forms-releases).

* **FORMS-23717** Efter uppgradering till **AEM Forms6.5.24.0**, `server.log` och `error.log` kan översvämmas av upprepade WARN-meddelanden, t.ex. *Det gick inte att skapa en säker parserfabrik* eller *Säkerhetsattribut ... stöds inte*. Loggar kan växa med cirka **5-10 rader per sekund** (hundratals MB per timme), vilket kan fylla i driftsättningen av disk- och blockproduktion.

Om du vill minska loggvolymen anger du loggningsnivån för `com.adobe.util.XMLSecurityUtil` till `ERROR` i programserverkonfigurationen eller via JVM-argumentet `-Dlogging.level.com.adobe.util.XMLSecurityUtil=ERROR`. Den här funktionen döljer bara meddelandena och löser inte den underliggande orsaken.
