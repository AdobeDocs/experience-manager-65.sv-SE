...
Du kommer att lära dig mer om dessa API:er och hur du använder dem i en senare del av den AEM resan utan headless. Du kan också läsa avsnittet [ytterligare resurser](#additional-resources) nedan om du vill ha mer information.
...
Den här integreringsnivån är den traditionella headless-modellen och gör det möjligt för innehållsförfattare att skapa innehåll i AEM och leverera det helhjärtat till ett valfritt antal externa tjänster med GraphQL eller redigera dem från externa tjänster med Assets API. Ingen kodning krävs i AEM.
...
Om målet är att skapa en SPA som konsumerar innehåll från AEM utan krångel kan du använda funktioner som Content Fragments för att hantera ditt headless-innehåll och även skapa en SPA med AEM SPA Editor-ramverk.
...
Du bör fortsätta din AEM resa utan att behöva besöka webbplatsen nästa gång du granskar dokumentet [Sökväg till din första upplevelse med AEM Headless](path-to-first-experience.md) där du får lära dig hur du konfigurerar de nödvändiga verktygen och hur du börjar fundera på att modellera data i AEM.
...
* [Namngivningskonventioner](/help/sites-developing/naming-conventions.md) - Teknisk dokumentation om namngivningsbegränsningar i AEM
