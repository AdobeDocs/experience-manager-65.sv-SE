...
SEO (Search Engine Optimization) har blivit en viktig fråga för många marknadsförare. Därför måste SEO:s problem hanteras i många AEM-projekt.

I det här dokumentet beskrivs först några [SEO-metodtips](#seo-best-practices) och rekommendationer för hur du kan uppnå dessa i en AEM-implementering. Sedan går det djupare in i några av de mer [komplexa implementeringssteg](#aem-configurations) som togs upp i det första avsnittet.
...
>[!NOTE]
>
>Du kan ange `sling:alias` egenskapen [Alias när du redigerar Sidegenskaper](/help/sites-authoring/editing-page-properties.md#advanced).
...
Det kavé som uppstår när du placerar `robots.txt` filen i platsroten är att begäranden om tömning av dispatcher kan ta bort filen och URL-mappningar troligtvis placerar platsroten någon annanstans än `DOCROOT` enligt definitionen i Apache HTTP Server-konfigurationen. Därför är det vanligt att placera den här filen på författarinstansen i platsroten och replikera den till publiceringsinstansen.
...
* Användare av din webbplats kan ha skapat bokmärken på dessa sidor. Genom att implementera omdirigeringar kan du vara säker på att dirigera användaren till sidan på den nya webbplatsen som mest liknar den plats där användaren försöker ta sig till den gamla webbplatsen.
