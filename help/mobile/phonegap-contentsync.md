...
>[!NOTE]
>
>Mer information om riktlinjer för utveckling av hanterare för innehållssynkronisering finns i avsnittet [Utveckla hanterare för innehållssynkronisering](/help/mobile/contentsync-app-handlers.md).
...
Om du vill skapa en konfiguration för innehållssynkronisering lägger du till en `cq:ContentSyncConfig`-nod i databasen med egenskapen `sling:resourceType` inställd på `contentsync/config`. Noden `cq:ContentSyncConfig` kan finnas var som helst i databasen, men noden måste vara tillgänglig för användare på AEM publiceringsinstans. Du bör därför lägga till noden under `/content`.
...
Om du [konfigurerar tjänsten med webbkonsolen](/help/sites-deploying/configuring-osgi.md#osgi-configuration-with-the-web-console) anger du namnet på användaren eller gruppen som värdet på egenskapen Authorizable för återställningscache.
...
Om du vill konfigurera hämtningsåtkomst för en specifik konfiguration för innehållssynkronisering lägger du till följande egenskap i noden `cq:ContentSyncConfig`:

* Namn: auktoriseringsbar
* Typ: String
* Värde: Namnet på den användare eller grupp som kan hämtas.

Med din app kan användare till exempel installera uppdateringar direkt från innehållssynkronisering. Om du vill att alla användare ska kunna hämta uppdateringen anger du värdet för den auktoriserbara egenskapen till `everyone`.

Om noden `cq:ContentSyncConfig` inte har någon auktoriseringsbar egenskap avgör standardanvändaren eller gruppen som är konfigurerad för auktoriseringsbar reservcache-egenskap för tjänsten för innehålls-Synkroniseringshanteraren för dag CQ vem som kan hämta.
...
Om cq:ContentSyncConfig-noden inte har någon `updateuser`-egenskap, uppdaterar den anonyma standardanvändaren cachen.
...
* **sökväg** - Om sökvägen pekar på en enda fil kopieras endast filen. Om den pekar på en mapp (inklusive sidnoder) kopieras alla filer och mappar nedan.

**content** - Återge innehåll med standardbehandling av Sling-begäran.

* **sökväg** - Sökväg till resurs som ska vara utdata.
* **extension** - Tillägg som ska användas i begäran. Vanliga exempel är *html* och *json*, men alla andra tillägg är möjliga.

* **väljare** - Valfria väljare avgränsade med punkt. Vanliga exempel är *touch* för återgivning av mobilversioner av en sida eller *infinity* för JSON-utdata.

**clientlib** - Paketera ett JavaScript- eller CSS-klientbibliotek.

* **sökväg** - Sökväg till klientbibliotekets rot.
* **extension** - Typ av klientbibliotek. Detta bör vara inställt på antingen *js* eller *css* för tillfället.

* **includeFolders** - Typen är en matris med strängar och låter användaren ange ytterligare mappar att skanna in i klientbiblioteket för att hämta filer (till exempel anpassade teckensnitt).

**resurser** - Samla in ursprungliga återgivningar av resurser.

* **sökväg** - sökväg till en resursmapp under /content/dam.
* **renditions** - Typen är en array med strängar där användaren kan ange vilka renderingar som ska användas i stället för standardbilden. I följande lista sammanfattas några färdiga återgivningar, men du kan även använda alla återgivningar som skapas i arbetsflödet:
...
**bild** - Samla in en bild.

* **sökväg** - sökväg till en bildresurs.
...
* **sökväg** - sökväg till en sida.
* **extension** - Tillägg som ska användas i begäran. För sidor är detta nästan alltid *html*, men andra är fortfarande möjliga.

* **väljare** - Valfria väljare avgränsade med punkt. Vanliga exempel är *touch* för återgivning av mobilversioner av en sida.

* **deep** - Valfri boolesk egenskap som även avgör om underordnade sidor ska inkluderas. Standardvärdet är *true.*

* **includeImages** - Valfri boolesk egenskap som avgör om bilder ska inkluderas. Standardvärdet är *true*.
Som standard beaktas endast bildkomponenter med en resurstyp för grund/komponenter/bild. Du kan lägga till fler resurstyper genom att konfigurera **Day CQ WCM Pages Update Handler** i webbkonsolen.

**rewrite** - Omskrivningsnoden definierar hur länkarna skrivs om på den exporterade sidan. De omskrivna länkarna kan antingen peka på filerna som finns i ZIP-filen eller på resurserna på servern.

Noden `rewrite` måste finnas under noden `page`.

Noden `rewrite` kan ha en eller flera av följande egenskaper:

* `clientlibs`: skriver om klientlibs-sökvägar.
...
* `REWRITE_RELATIVE`: skriver om sökvägen med en relativ position till sidfilen .html i filsystemet.

* `REWRITE_EXTERNAL`: skriver om sökvägen genom att peka på resursen på servern med hjälp av AEM [Externalizer-tjänsten](/help/sites-developing/externalizer.md).

Med AEM **PathRewriterTransformerFactory** kan du konfigurera de specifika HTML-attribut som ska skrivas om. Tjänsten kan konfigureras i webbkonsolen och har en konfiguration för varje egenskap för noden `rewrite`: `clientlibs`, `images` och `links`.
...
**etc.designs.default och osv.designs.mobile** - de två första posterna i konfigurationen är uppenbara. Eftersom vi ska ta med flera mobilsidor behöver vi de relaterade designfilerna nedan /etc/designs. Och eftersom ingen extra bearbetning behövs räcker det med en kopia.

**events.plist** - Den här posten är lite speciell. Som nämndes i inledningen bör programmet tillhandahålla en kartvy med markörer för händelsernas platser. Vi kommer att tillhandahålla nödvändig platsinformation som en separat fil i PLIST-format. För att detta ska fungera har händelselistkomponenten som används på indexsidan ett skript som heter plist.jsp. Det här skriptet körs när komponentens resurs begärs med tillägget `.plist`. Som vanligt anges komponentsökvägen i egenskapen path och typen ställs in på content eftersom vi vill använda bearbetningen av Sling-begäran.

**events.touch.html** - Här kommer de faktiska sidorna som visas i appen. Egenskapen path ställs in på händelsens rotsida. Alla händelsesidor under den sidan inkluderas också eftersom egenskapen deep har standardvärdet true. Vi använder sidor som konfigurationstyp, så att alla bilder eller andra filer som kan refereras från en bild eller en nedladdningskomponent på en sida inkluderas. Om du dessutom ställer in pekväljaren får vi en mobilversion av sidorna. Konfigurationen i funktionspaketet innehåller fler poster av den här typen, men utelämnas för enkelhetens skull här.
...
**manifest** - Det är ofta önskvärt att inkludera någon typ av metadata i zip-filen, till exempel startsidan för ditt innehåll. Om du däremot hårdkodar sådan information kan du inte ändra den senare. Innehållssynkroniseringsramverket stöder det här användningsexemplet genom att söka efter en manifestnod i konfigurationen, som identifieras med namn och som inte kräver någon konfigurationstyp. Alla egenskaper som definieras på den aktuella noden läggs till i en fil, som också kallas manifest och finns i zip-filens rot.

I exemplet ska händelselistsidan vara den första sidan. Den här informationen anges i egenskapen **indexPage** och kan därför enkelt ändras när som helst. En andra egenskap definierar sökvägen till filen *events.plist* . Som vi ser senare kan klientprogrammet nu läsa manifestet och agera utifrån det.

När konfigurationen är konfigurerad kan innehållet hämtas med en webbläsare eller en annan HTTP-klient, eller om du utvecklar för iOS kan du använda det dedikerade WAppKitSync-klientbiblioteket. Hämtningsplatsen består av konfigurationens sökväg och tillägget *.zip*, till exempel när du arbetar med en lokal AEM: *https://localhost:4502/content/weretail_go.zip*
...
Registrera klassen som OSGi-komponentfabrik och distribuera den i OSGi-behållaren i ett paket. Detta kan göras med [plugin-programmet för Maven SCR](https://felix.apache.org/documentation/subprojects/apache-felix-maven-scr-plugin/apache-felix-maven-scr-plugin-use.html) med JavaDoc-taggar eller anteckningar. I följande exempel visas JavaDoc-versionen:
...
>[!CAUTION]
>
>När du bygger vidare på basklassen AbstractSlingResourceUpdateHandler måste du lägga till definitionen *inherit*. Annars kommer OSGi-behållaren inte att ange de nödvändiga referenserna som deklareras i basklassen.

### Implementera en anpassad uppdateringshanterare {#implementing-a-custom-update-handler}

Varje sida för e-butiksmobilen innehåller en logotyp i det övre vänstra hörnet som vi vill inkludera i zip-filen. För cacheoptimering refererar AEM inte till bildfilens verkliga plats i databasen, vilket förhindrar oss från att använda konfigurationstypen **copy**. I stället måste vi tillhandahålla en egen konfigurationstyp för **logotyp** som gör bilden tillgänglig på den plats som AEM begär. I följande kodexempel visas den fullständiga implementeringen av hanteraren för uppdatering av logotyp:
...
Klassen `LogoUpdateHandler` implementerar `ContentUpdateHandler`-gränssnittets `updateCacheEntry(ConfigEntry, Long, String, Session, Session)`-metod, som tar flera argument:

* En `ConfigEntry`-instans som ger åtkomst till konfigurationsposten, som hanteraren anropas för, och dess egenskaper.
* En `lastUpdated`-tidsstämpel som anger den senaste gången som Content Sync uppdaterade sin cache. Innehåll som inte har ändrats efter den tidsstämpeln ska inte uppdateras av hanteraren.
* Ett `configCacheRoot`-argument som anger rotsökvägen för cachen. Alla uppdaterade filer måste lagras under den här sökvägen för att kunna läggas till i zip-filen.
