...
1. Använd listrutan **Varumärke** för att välja det varumärke som du vill skapa aktiviteten för.
   ...
   >[!NOTE]
   >
   >Du bör [skapa varumärken via aktivitetskonsolen](/help/sites-authoring/activitylib.md#creating-a-brand-using-the-activities-console).
   >
   >
...
1. Välj målmotor i listrutan **Målmotor**.
   ...
   * Om du väljer **Adobe Target** kan du välja en konfiguration (som standard är det den konfiguration som du angav när du [konfigurerade kontot](/help/sites-administering/opt-in.md)) och aktivitetstypen.
...
När du har [påbörjat målprocessen](/help/sites-authoring/content-targeting-touch.md#the-targeting-process-create-target-and-goals-settings) väljer du en upplevelse för att se erbjudandena som finns för den upplevelsen. När du väljer en upplevelse ändras målkomponenterna på sidan så att erbjudandet för den upplevelsen visas.
...
Utför följande procedur när [målprocessen](/help/sites-authoring/content-targeting-touch.md#the-targeting-process-create-target-and-goals-settings) har startats.
...
>[!NOTE]
>
>Erbjudanden som du skapar med [Erbjudandekonsolen](/help/sites-authoring/offerlib.md) kan även innehålla flera komponenter. Erbjudandena tillhör ett erbjudandebibliotek och kan användas för flera olika upplevelser.
...
1. Som standard har inget plats-ID angetts. Klicka på eller tryck på konfigurationshjulet för att ange platsen.
   ...
   >
   >
   >Administratörer kan bestämma om den här konfigurationen behöver anges på **https://&lt;host>:&lt;port>/system/console/configMgr/com.day.cq.personalization.impl.servlets.TargetingConfigurationServlet**
...
Målsteget [målprocessen](/help/sites-authoring/content-targeting-touch.md#the-targeting-process-create-target-and-goals-settings) innebär att mappa målgrupper med de upplevelser du arbetade med i steget Skapa. Målsidan visar vilka målgrupper varje upplevelse riktar sig till. Ni kan ange eller ändra målgruppen för varje upplevelse. Om du använder Adobe Target kan du även skapa A/B-tester som gör att du kan ange en procentandel av trafiken för en viss målgrupp för en viss upplevelse.
...
Målet och inställningarna i [målsättningsprocessen](/help/sites-authoring/content-targeting-touch.md#the-targeting-process-create-target-and-goals-settings) innebär att varumärkesaktivitetens beteende konfigureras. Ange när aktiviteten startar och avslutas samt aktivitetsprioriteten. Dessutom håller du också koll på målen. Du kan bestämma vad du vill mäta med dina aktiviteter.
 ...
 <tbody>
  <tr>
   ...
   <td><p>Andelen besökare som klickade på någon del av upplevelsen som testades. En konvertering kan antingen räknas en gång per besökare eller varje gång en besökare slutför en konvertering. Konverteringsmåttet är inställt på något av följande:</p>
    <ul>
     <li><strong>Visad sida</strong> - Du kan definiera vilken sida målgruppen ska visa genom att markera antingen  <strong>URL-adressen </strong> och sedan definiera URL-adressen eller flera URL-adresser, eller genom att markera  <strong>URL-</strong> behållare och sedan lägga till en sökväg eller ett nyckelord.</li>
     <li><strong>Visad mbox</strong> - Du kan definiera vilken mbox som publiken ska visa genom att ange namnet på mbox. Du kan ange flera rutor genom att klicka på <strong>Lägg till en Mbox</strong>.</li>
    </ul> </td>
  </tr>
  <tr>
...
När du har konfigurerat kan du [visa resultatet för dina aktiviteter](/help/sites-authoring/activitylib.md#viewing-performance-and-converting-winning-experiences-a-b-test) som använder Adobe Target (antingen upplevelse eller A/B-testmål). Med A/B-tester som mål kan du dessutom [konvertera vinnarna.](/help/sites-authoring/activitylib.md#viewing-performance-and-converting-winning-experiences-a-b-test)
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>Klicka eller tryck på <strong>Lägg till fält</strong> för att konfigurera ytterligare kontextparametrar (samma som det som är tillgängligt i Target Framework). Sammanhangsparametrar som läggs till i komponenten använder <i>bara</i> på komponenten och inte på andra komponenter, vilket skulle vara fallet om du lade till kontextparametrar direkt i ramverket.</td>
  </tr>
  <tr>
   <td><strong>Statiska parametrar</strong></td>
   <td>Klicka eller tryck på <strong>Lägg till fält</strong> för att konfigurera ytterligare statiska parametrar (samma som det som är tillgängligt i Target Framework). Statiska parametrar som läggs till i komponenten använder <i>bara</i> på komponenten och inte på andra komponenter, vilket skulle vara fallet om du lade till statiska parametrar direkt i ramverket. Statiska parametrar kommer inte från kontext (klientkontext för innehållsnavet).</td>
  </tr>
 </tbody>
</table>
