...
seo-description: WCAG 2.0 består av en uppsättning teknikoberoende riktlinjer och framgångskriterier som gör webbinnehåll tillgängligt för och användbart för personer med funktionshinder.
...
* Arkiv:
   ...
   En bild överförs från användarens hårddisk. Om du lägger till en bildkomponent på en sida och sedan väljer en bild från hårddisken, eller en annan källa, används standardvärdet för **Alt-text** `file`. Detta måste ändras på fliken **Avancerade** bildegenskaper. Detta värde visas inte i fältet **Alt-text** , men när värdet ändras visas det nya värdet i fältet.
...
>[!NOTE]
>
>I båda dessa scenarier visas inte standardvärdet för **Alt-text** på fliken **Avancerade bildegenskaper** . Om du vill ändra standardvärdet anger du ett nytt värde i fältet **Alt-text** .

>[!NOTE]
>
>Om bilden är helt dekorativ (se [Skapa bra textalternativ](#creating-good-text-alternatives)) kan du ange ett blanksteg i fältet **Alt-text** med hjälp av blankstegstangenten. Detta skapar ett tomt `alt` attribut som uppmanar en skärmläsare att ignorera bilden.
...
* Illustrativa foton:
    ...
    Det här är bilder på människor, objekt eller platser. Tänk på fotots roll på sidan; en lämplig textmotsvarighet är sannolikt *Foto av[objekt ]*, men kan vara beroende av den omgivande texten.
...
AEM gör det enkelt att skapa webbsidor med rätt HTML-element. Öppna sidinnehållet i textredigeraren (en textkomponent) och använd menyn **Format** för att ange rätt strukturelement (t.ex. stycke, rubrik osv.).
...
* **Använda rubriker:**
   ...
   Så länge du har tillgänglighetsfunktionerna i RTE aktiverade (se [AEM och tillgänglighet](/help/sites-administering/rte-accessible-content.md)) erbjuder AEM tre nivåer för sidrubriken. Du kan använda dessa för att identifiera avsnitt och underavsnitt av innehåll. Rubrik 1 är den högsta rubriknivån, rubrik 3 den lägsta. Systemadministratören kan konfigurera systemet så att fler rubriknivåer tillåts.
...
* **Använd tabeller**:
   ...
   * ett `<td>` element för varje datacell
   >[!NOTE]
   Tabeller ska realiseras med **tabellkomponenten** . Även om tabeller kan skapas i Text-komponenten rekommenderas inte detta.
...
Du kan också redigera sidans titel när du redigerar en sida, som du kommer åt via **Sidspark** - fliken **Sida** - **Sidegenskaper...**
