...
description: Bästa metoder för att fastställa effektiva mätvärden för att uppskatta den infrastruktur och de resurser som krävs för att driftsätta Adobe Experience Manager Assets.
contentOwner: AG
...
När miljön för en implementering av Adobe Experience Manager Assets storleksanpassas är det viktigt att se till att det finns tillräckligt med resurser tillgängliga när det gäller disk, processor, minne, IO och nätverksgenomströmning. Om du vill ändra storlek på många av dessa resurser måste du känna till hur många resurser som läses in i systemet. Om det inte finns något bättre mätvärde kan du dividera storleken på det befintliga biblioteket med bibliotekets ålder för att hitta frekvensen som resurserna skapas med.

## Skiva {#disk}

### DataStore {#datastore}

Ett vanligt misstag när storleken på det nödvändiga diskutrymmet för en resursimplementering väljs är att beräkningarna baseras på storleken på de råbilder som ska importeras till systemet. Som standard skapar Experience Manager tre återgivningar utöver den ursprungliga bilden som kan användas för att återge gränssnittselementen i Experience Manager. I tidigare implementeringar har dessa återgivningar observerats anta dubbelt så stora som storleken på de resurser som har importerats.

De flesta användare definierar anpassade återgivningar utöver de färdiga återgivningarna. Förutom återgivningarna kan du med Assets extrahera underresurser från vanliga filtyper, till exempel [!DNL Adobe InDesign] och [!DNL Adobe Illustrator].

Versionshanteringsfunktionerna i Experience Manager lagrar kopior av resurserna i versionshistoriken. Du kan konfigurera versionerna så att de rensas ofta. Många användare väljer dock att behålla versioner i systemet under lång tid, vilket kräver ytterligare lagringsutrymme.
...
1. Bestäm hur ofta befintliga resurser ändras i systemet. Om Experience Manager används som ett samarbetscentrum i kreativa arbetsflöden är antalet ändringar höga. Om endast färdiga resurser överförs till systemet är det här antalet mycket lägre.
...
Du kan ange dessa tal i kalkylbladet Nätverksstorlek för att fastställa det totala utrymmet som krävs för datalagret. Det är också ett användbart verktyg för att avgöra vilken effekt det har på disktillväxten att underhålla resursversioner eller ändra resurser i Experience Manager.
...
Resurser har ett antal användningsfall som gör nätverksprestanda viktigare än många av våra Experience Manager-projekt. En kund kan ha en snabb server, men om nätverksanslutningen inte är tillräckligt stor för att stödja belastningen på de användare som överför och hämtar resurser från systemet verkar den ändå vara långsam. Det finns en bra metod för att fastställa krympningspunkten i en användares nätverksanslutning till Experience Manager vid [Assets för användarupplevelser, instansstorlek, utvärdering av arbetsflöde och nätverkstopologi](/help/assets/assets-network-considerations.md).
...
Det är svårt att exakt uppskatta storleken på TIFF-filen som stöds direkt med en särskild heap för Experience Manager eftersom ytterligare faktorer, som pixelstorlek, påverkar bearbetningen. Det är möjligt att Experience Manager kan bearbeta en fil som är 255 MB färdig, men inte kan bearbeta en filstorlek på 18 MB eftersom den senare innehåller ett ovanligt högre antal pixlar än den förra.

## Storlek på resurser {#size-of-assets}

Som standard kan du med Experience Manager överföra resurser med en filstorlek på upp till 2 GB. Information om hur du överför mycket stora resurser i Experience Manager finns i [Konfiguration för att överföra mycket stora resurser](managing-video-assets.md#configuration-to-upload-assets-that-are-larger-than-gb).
