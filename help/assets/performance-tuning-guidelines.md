...
Adobe rekommenderar att du distribuerar [!DNL Experience Manager Assets] på Java 8 för optimala prestanda.
...
1. Välj **[!UICONTROL Transient Workflow]** och klicka på **[!UICONTROL OK]**.
...
1. Gå till `https://[aem_server]:[port]/system/console/slingevent` på [!DNL Experience Manager] Author.

1. Klicka på **[!UICONTROL Edit]** för varje arbetsflödeskö som är relevant för implementeringen, till exempel **[!UICONTROL Granite Transient Workflow Queue]**.
...
Om du anpassar arbetsflödet [!UICONTROL DAM Update Asset] för att generera renderingar med ImageMagick rekommenderar Adobe att du ändrar `policy.xml`-filen på `/etc/ImageMagick/`. Som standard använder ImageMagick hela det tillgängliga diskutrymmet på operativsystemsvolymen och det tillgängliga minnet. Gör följande konfigurationsändringar i `policymap`-avsnittet i `policy.xml` för att begränsa de här resurserna.
...
1. Bläddra till `/oak:index/damAssetLucene`. Lägg till en `String[]`-egenskap `includedPaths` med värdet `/content/dam`. Spara ändringar.
