...
[!DNL Experience Manager] har vissa standardarbetsflöden för att bearbeta resurser. Om du vill visa dem öppnar du arbetsflödeskonsolen och klickar på fliken **[!UICONTROL Models]**: de arbetsflödesrubriker som börjar med [!DNL Assets] är resursspecifika.
...
Se [Utvecklingsverktyg](../sites-developing/dev-tools.md) för att installera och konfigurera Eclipse med ett [!DNL Maven]-plugin-program och för att ställa in beroenden som krävs för [!DNL Maven]-projektet.
...
1. Skapa `myBundle` [!DNL Maven]-projekt i Eclipse:
   ...
   1. Klicka på **[!UICONTROL File]** > **[!UICONTROL New]** > **[!UICONTROL Other]** i menyfältet.
   1. Expandera mappen [!DNL Maven] i dialogrutan, välj [!DNL Maven]-projektet och klicka på **[!UICONTROL Next]**.
...
1. Skapa klassen [!DNL Java] `MyHandler`:
   ...
   1. I [!DNL Eclipse], under `myBundle/src/main/java`, högerklickar du på `com.day.cq5.myhandler`-paketet. Välj [!UICONTROL New] och sedan [!UICONTROL Class].
   1. I dialogrutan ger du klassen [!DNL Java] namnet `MyHandler` och klickar på [!UICONTROL Finish]. [!DNL Eclipse] skapar och öppnar filen  `MyHandler.java`.
...
* Flyttar tillbaka resultatet till återgivningsmappen för [!DNL Experience Manager]-servern.
...
I följande exempel visas hur du ställer in kommandoradsprocesssteget så att varje gång en resurs med e-typen miMIME GIF eller TIFF läggs till i `/content/dam` på [!DNL Experience Manager]-servern, skapas en vänd bild av originalet tillsammans med tre ytterligare miniatyrbilder (140x100, 48x48 och 10x2) 50).
...
Installera [!DNL ImageMagick] på den disk som är värd för [!DNL Experience Manager]-servern:
...
1. Om du vill se om verktyget är korrekt installerat kör du följande kommando `convert -h` på kommandoraden.
   ...
   >[!NOTE]
   >
   >I vissa versioner av Windows kan kommandot convert misslyckas eftersom det står i konflikt med det inbyggda konverteringsverktyget som ingår i [!DNL Windows]-installationen. I det här fallet anger du den fullständiga sökvägen för [!DNL ImageMagick]-programmet som används för att konvertera bildfiler till miniatyrer. Till exempel, `"C:\Program Files\ImageMagick-6.8.9-Q16\convert.exe" -define jpeg:size=319x319 ${filename} -thumbnail 319x319 cq5dam.thumbnail.319.319.png`.

1. Om du vill se om verktyget fungerar som det ska lägger du till en JPG-bild i arbetskatalogen och kör kommandot convert `<image-name>.jpg -flip <image-name>-flipped.jpg` på kommandoraden. En speglad bild läggs till i katalogen. Lägg sedan till kommandoradsprocesssteget i **[!UICONTROL DAM Update Asset]**-arbetsflödet.
...
Avgränsa värdena för [!UICONTROL Process Arguments] med kommatecken och starta det inte med ett blanksteg.
...
Om till exempel [!DNL ImageMagick] är installerat på den disk som är värd för [!DNL Experience Manager]-servern och du skapar ett processsteg med [!UICONTROL CommandLineProcess] som implementering och följande värden är [!UICONTROL Process Arguments]:
