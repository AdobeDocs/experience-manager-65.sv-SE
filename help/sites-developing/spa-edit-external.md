...
* `model` - Ett  `model` objekt som skickas som en parameter som vanligtvis fylls i när  [SSR används.](spa-ssr.md)
...
Om det finns flera sidor i det externa SPA kan det [använda routning för att avgöra vilken sida/komponent som ska återges.](spa-routing.md) Det grundläggande användningsexemplet är att matcha den aktiva URL:en mot sökvägen som anges för ett flöde. Om du vill aktivera redigering för sådana routningsaktiverade program måste sökvägen som ska matchas mot omformas för att rymma AEM information.
