...
När [du anpassar och utökar dina arbetsflöden](/help/sites-developing/workflows-customizing-extending.md) får du åtkomst till arbetsflödesobjekt:
...
## Använda arbetsflödets REST API {#using-the-workflow-rest-api}
...
#### Hantera en arbetsflödesinstans efter dess tillstånd {#managing-a-workflow-instance-by-its-state}
...
<table>
 <tbody>
  ...
  <tr>
   <td><code>POST</code></td>
   <td>Slutför det arbetsobjekt vars URI skickas som parametern <code>item</code> och flyttar arbetsflödesinstansen till nästa nod(er), som definieras av parametern <code>route</code> eller <code>backroute</code> om ett steg bakåt inträffar.<br /> Om parametern  <code>delegatee</code> skickas  <code>item</code> delegeras arbetsposten som identifieras av parametern till den angivna deltagaren.</td>
  </tr>
 </tbody>
</table>
...
#### Hämta en lista över alla arbetsflöden som körs med deras ID:n - REST med kurl {#how-to-get-a-list-of-all-running-workflows-with-their-ids-rest-using-curl}
...
`uri` som visas i resultatet kan användas som instans `id` i andra kommandon; till exempel:
...
>[!NOTE]
>
>Det här `curl`-kommandot kan användas med valfri [arbetsflödesstatus](/help/sites-administering/workflows.md#workflow-status-and-actions) i stället för `RUNNING`.
...
### Så här listar du alla arbetsflödesmodeller {#how-to-list-all-workflow-models}

Om du vill visa en lista över alla tillgängliga arbetsflödesmodeller gör du en GET till:

`http://localhost:4502/etc/workflow/models.json`

#### Så här listar du alla arbetsflödesmodeller - REST med kurvan {#how-to-list-all-workflow-models-rest-using-curl}
...
* Koden för Java- och ECMA-skriptet använder metoden `WorkflowSession.createNewModel`.
...
>[!NOTE]
>
>Om du tar bort modellen anges egenskapen `deleted` för modellens underordnade `metaData`-nod till `true`.
>
...
* Argumentet `id` som vissa metoder kräver för att identifiera modellen är den absoluta sökvägen till modellnoden i databasen:
...
#### Tar bort en arbetsflödesmodell - REST med kurvan {#deleting-a-workflow-model-rest-using-curl}
...
Om du behöver gå vidare automatiskt till ett **deltagarsteg** som inte har slutförts inom en fördefinierad tid kan du:
...
#### Interagera med arbetsflödesinstanser - ECMA-skript {#interacting-with-workflow-instances-ecma-script}
...
#### Interaktion med arbetsflödesinstanser - REST med kursen {#interacting-with-workflow-instances-rest-using-curl}
...
* **Visar instanser**
   ...
   >[!NOTE]
   >
   >Se [Så här hämtar du en lista över alla arbetsflöden som körs](#how-to-get-a-list-of-all-running-workflows-with-their-ids) med deras ID:n för att lista instanser med en viss status.
...
### Samverka med arbetsobjekt {#interacting-with-work-items}

Nedan följer grundläggande exempel på hur du interagerar (programmatiskt) med arbetsobjekt.

#### Interagera med arbetsobjekt - Java {#interacting-with-work-items-java}
...
#### Interagera med Work Items - ECMA Script {#interacting-with-work-items-ecma-script}
...
#### Interagera med arbetsobjekt - REST med kursen {#interacting-with-work-items-rest-using-curl}
