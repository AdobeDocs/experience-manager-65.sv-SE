...
**Adobe XFA Forms Renderer (com.adobe.livecycle.adobe-lc-forms-core)** är visningsnamnet för HTML5-formulären i OSGi-paketet när det visas från Bundle View i Felix Admin Console (https://[host]:[port]/system/console/bundles).
...
Denna OSGi-tjänst innehåller logik för att återge en XDP som HTML och hanterar överföringen av ett formulär för att generera data-XML. Den här tjänsten använder Forms tjänstbehållare. Forms tjänstbehållare anropar den interna komponenten `XMLFormService.exe` som utför bearbetningen.
...
Skriptmotorn i HTML Forms är skriven i JavaScript för att stödja XFA-skriptprogrammeringsgränssnittet på båda dessa språk.
