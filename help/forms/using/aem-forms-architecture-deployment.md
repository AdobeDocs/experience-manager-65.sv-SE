...
* **Webblager:** JSP eller serverlets, som bygger på vanliga tjänster och blanketttjänster och som har följande funktioner:
   ...
   * **Formuläråtergivning och inskickning**: Ett användargränssnitt för slutanvändare som används av AEM Forms slutanvändare (t.ex. medborgare som använder en myndighets webbplats). Detta ger formuläråtergivning (visa formulär i en webbläsare) och funktioner för att skicka in formulär.
...
**AEM Forms on OSGi:** En AEM Forms i OSGi-miljö är AEM Author eller AEM Publish med AEM Forms-paket som används som standard. Du kan köra AEM Forms på OSGi i en [enda servermiljö, Farm och grupperade konfigurationer](/help/sites-deploying/recommended-deploys.md). Klusterinställningar är bara tillgängliga för instanser av AEM Author.

**AEM Forms på JEE:** AEM Forms på JEE är AEM Forms-server som körs på JEE-stacken. Den har AEM Author med tilläggspaket för AEM Forms och ytterligare JEE-funktioner för AEM Forms som distribueras samtidigt på en enda JEE-stack som körs på en programserver. Du kan köra AEM Forms på JEE i enserver- och gruppkonfigurationer. AEM Forms on JEE krävs endast för dokumentsäkerhet, processhantering och för LiveCycle-kunder som uppgraderar till AEM Forms. Här följer några ytterligare scenarier för användning av AEM Forms på JEE:

* **Stöd för HTML-arbetsyta (för kunder som använder HTML-arbetsytan):** AEM Forms på JEE möjliggör enkel inloggning med bearbetningsinstanser, visar vissa resurser som återges med bearbetningsinstanser och hanterar överföringen av formulär som återges på HTML-arbetsytan.
...
* **Integrerad användarhantering:** Tillåter användare av AEM Forms på JEE att identifieras som AEM-formulär för OSGi-användare och möjliggör enkel inloggning för både OSGi- och JEE-användare. Detta krävs för scenarier där enkel inloggning mellan AEM-formulär på OSGi och AEM Forms på JEE krävs (till exempel HTML-arbetsyta).
* **Värdtjänst:** AEM Forms på JEE kan hantera resurser (till exempel HTML5-formulär) som återges på AEM Forms i OSGi.

Användargränssnittet för att skapa AEM Forms stöder inte skapande av DOR- (Document of Record), PDF forms- och HTML5-formulär. Sådana resurser är utformade med det fristående Forms Designer-programmet och överförs individuellt till AEM Forms Manager. För AEM Forms på JEE kan formulär även utformas som programresurser (i AEM Forms Workbench) och distribueras till AEM Forms på JEE-server.

AEM Forms på OSGi och AEM Forms på JEE har båda arbetsflödesfunktioner. Du kan snabbt skapa och distribuera grundläggande arbetsflöden för olika uppgifter i AEM-formulären på OSGi, utan att behöva installera den kompletta processhanteringsfunktionen i AEM Forms på JEE. Det finns en viss skillnad i [funktionerna för formulärcentrerat arbetsflöde på AEM Forms i OSGi och processhantering i AEM Forms i JEE](capabilities-osgi-jee-workflows.md). Utvecklingen och hanteringen av formulärcentrerade arbetsflöden på AEM Forms i OSGi använder det välbekanta arbetsflödet i AEM Workflow och AEM Inbox.

## Terminologies {#terminologies}

Följande bild visar olika AEM Form-serverkonfigurationer och deras komponenter som används i en typisk AEM Forms-distribution:
...
AEM Forms som planerar att använda processhanteringsfunktionerna i AEM Forms kan t.ex. ha en topologi som liknar den som visas nedan. AEM Forms på JEE-servern kan vara i en enda server- eller klusterkonfiguration.

Om du uppgraderar från LiveCycle ES4 speglar den här topologin noga det du redan har i LiveCycle, förutom att AEM Author är inbyggt i AEM Forms på JEE. Dessutom ändras inte kraven på klustring för kunder som utför en uppgradering. Om du använde AEM Forms i en klustermiljö kan du fortsätta med samma sak i AEM 6.5-formulär. För en ny installation av AEM Forms av JEE för HTML-arbetsyta är körning av AEM-författarinstansen inbyggd i JEE-miljön ytterligare ett krav.
...
AEM Forms som planerar att använda datainhämtningsfunktionerna för AEM Forms, till exempel adaptiva formulär, HTML5-formulär, PDF forms, kan ha en topologi som liknar den som visas nedan. Denna topologi rekommenderas också för användning av interaktiva kommunikationsfunktioner i AEM Forms.
...
* Om du vill använda HTML Workspace och AEM Forms-appen måste du ha en AEM-författare eller bearbetningsinstans. Du kan använda den AEM-författarinstans som är inbyggd i AEM Forms på JEE-servern i stället för att konfigurera en extra extern AEM-författarserver.
* En AEM Author- eller Bearbetningsinstans krävs bara för formulärbaserade arbetsflöden i OSGi, adaptiva formulär, formulärportalen och interaktiv kommunikation.
...
* AEM-formulär på OSGi-instanser som är inbyggda i AEM Forms på JEE-servern kan även köra formulärorienterade arbetsflöden på OSGi och Bevakade mappar.
