...
**jQuery** pluginTillhandahåller en standardmekanism, baserad på jQuery-widgetramverket, för att implementera ett alternativt utseende.

**** ClientLibEtt bibliotekssystem på klientsidan AEM klientbearbetning som styrs av komplex JavaScript- och CSS-kod. Mer information finns i Använda bibliotek på klientsidan.

**Arketype** Ett Maven-projekt med mallverktyg som definieras som ett ursprungligt mönster eller en modell för Maven-projekt. Mer information finns i Introduktion till arkitekturer.

**User** ControlRefererar till huvudelementet i en widget som innehåller fältets värde och används av utseenderamverket för att binda det anpassade widgetgränssnittet med den adaptiva formulärmodellen.
...
1. Inkludera plugin-programmets beroende från tredje part i projektet.
   ...
   1. Placera jQuery-plugin-program från tredje part eller anpassade program i mappen `jqueryplugin/javascript` och relaterade CSS-filer i mappen `jqueryplugin/css`. Mer information finns i JS- och CSS-filerna i mappen `jqueryplugin/javascript and jqueryplugin/css`.

   1. Ändra `js.txt`- och `css.txt`-filerna så att de innehåller ytterligare JavaScript- och CSS-filer för jQuery-plugin-programmet.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>Som standard visas i XFA vid enter-händelse <code>rawValue</code> för fältet. Den här funktionen anropas för att visa <code>rawValue</code> för användaren. </td>
  </tr>
  <tr>
   <td><code>showDisplayValue</code></td>
   <td>Som standard visas i XFA vid exit-händelse <code>formattedValue</code> för fältet. Den här funktionen anropas för att visa <code>formattedValue</code> för användaren. </td>
  </tr>
 </tbody>
</table>
...
>[!NOTE]
>
>Maven-projektet refererar till en fjärrdatabas inuti POM-filen. Detta är bara för referensändamål, och enligt Maven-standard hämtas databasinformationen i `settings.xml`-filen.
...
1. Öppna Eclipse-verktyget och gör följande för att importera Eclipse-projektet:
   ...
   1. Bläddra och markera den mapp där du utförde kommandot `archetype:generate`.
...
1. Välj den widget som ska användas för det anpassade utseendet. I det här exemplet används följande numeriska nummerwidget:
   ...
   Granska plugin-programkoden i `plugin.js`-filen i Eclipse-projektet för att se om den matchar utseendekraven. I det här exemplet uppfyller utseendet följande krav:

   * Den numeriska nummerlistan ska sträcka sig från `- $.xfaWidget.numericInput`.
   * Metoden `set value` i widgeten anger värdet efter att fokus är på fältet. Det är ett obligatoriskt krav för en widget för anpassningsbara formulär.
   * Metoden `render` måste åsidosättas för att du ska kunna anropa metoden `bootstrapNumber`.
...
1. I filen `numericStepper-widget.js` lägger du till följande kod för att åsidosätta återgivningsmetoden för att anropa plugin-programmet och returnera `$userControl`-objektet:
...
1. Öppna det adaptiva formuläret i redigeringsläge som du vill använda det anpassade utseendet på och gör följande:
   ...
   1. Högerklicka på det fält där du vill använda utseendet och klicka på **[!UICONTROL Edit]** för att öppna dialogrutan Redigera komponent.
