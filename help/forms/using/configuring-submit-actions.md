...
Parametrar som du använder för att skicka XML-data och bifogade filer är `dataXml` och `attachments`.
...
Alternativet **Skicka till Forms Workflow** skickar en data-xml och eventuella bifogade filer till en befintlig Adobe LiveCycle eller AEM Forms i JEE-process.
...
[Konfigurera Experience Manager DS-inställningarna](../../forms/using/configuring-the-processing-server-url-.md) innan du använder **AEMSkicka-åtgärden**. Mer information om hur du skapar ett AEM arbetsflöde finns i [Formulärbaserade arbetsflöden i OSGi](../../forms/using/aem-forms-workflow.md).
