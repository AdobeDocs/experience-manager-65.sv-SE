...
**Fyll sida med:** När det här alternativet är markerat fyller i attribut på användar- och gruppinställningssidorna med motsvarande LDAP-standardvärden.

**Hämta basens unika namn:** Hämtar basens unika namn och visar dem i listrutan. Den här inställningen är användbar när du har flera bas-DN och behöver välja ett värde.
...
Om du valde alternativet Aktivera hänvisning i kataloginställningarna anger du värdet för alternativet Base DN till *dc*-delen av DN. För att referensen ska fungera måste sökintervallet innehålla både överordnade och underordnade domäner.
...
**Base-DN:** (obligatoriskt) Katalogens unika namn.
...
**Organisation:** Schemaattribut för namnet på den organisation som gruppen tillhör
...
**Sortera fältnamn:** Om du valde Aktivera VLV-kontroll (Virtual List View) anger du det attributnamn som används för att sortera indexet. Det här attributnamnet är det som du angav när du skapade ett index för VLV på katalogservern.
...
1. Exempelskriptet har en LDAP-post med namnet `lcuser`. Den här posten är avsedd för VLV-relaterad konfiguration för användarsynkronisering i AEM formulär. Ändra följande egenskaper i enlighet med detta:
   ...
   **Postnamn:** Postnamnet i det här exemplet är  `lcuser`. Om `lcuser` ändras måste den ändras i alla delar av exempelskriptet.

   **vlvBase:** Basens unika namn som anges på sidan Användarinställningar.
