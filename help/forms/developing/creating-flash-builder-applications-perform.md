...
När ett klientprogram startas kan du göra en POST-förfrågan till `/um/login`-säkerhetsservern. Exempel: `https://<your_serverhost>:<your_port>/um/login?um_no_redirect=true`. När begäran når användarhanterarens säkerhetstjänst utför den följande steg:
...
>[!NOTE]
>
>Medan `authstate=CREDENTIAL_CHALLENGE` är rekommenderat att klienten skickar de inhämtade autentiseringsuppgifterna till säkerhetsservern i ett POST-formulär.
...
* `SSOStandalone.mxml`: Den MXML huvudfilen som representerar klientprogrammet. (Se [Skapa filen SSOStandalone.mxml](creating-flash-builder-applications-perform.md#creating-the-ssostandalone-mxml-file).)
