...
seo-description: Använd tjänsten för integrering av formulärdata för att importera data till ett PDF-formulär och exportera data från ett PDF-formulär med Java API och Web Service API.
...
* Ett Adobe XML-formulär (skapat i Designer) är ett PDF-dokument som följer XML Adobe XML Forms Architecture (XFA).
...
## Importerar formulärdata {#importing-form-data}
...
1. Referera till en XML-datakälla.
   ...
   * Skapa ett `com.adobe.idp.Document`-objekt som lagrar formulärdata med konstruktorn `com.adobe.idp.Document`. Skicka `java.io.FileInputStream`-objektet som innehåller formulärdata till konstruktorn.
...
1. Importera data till PDF-formuläret.
   ...
   * Det `com.adobe.idp.Document`-objekt som lagrar formulärdata.
...
1. Spara PDF-formuläret som en PDF-fil.
   ...
   * Anropa `Document`-objektets `copyToFile`-metod för att kopiera innehållet i `Document`-objektet till filen (kontrollera att du använder `Document`-objektet som returnerades av metoden `importData`).
...
### Importera formulärdata med webbtjänstens API {#import-form-data-using-the-web-service-api}
...
1. Skapa en tjänstklient för integrering av formulärdata.
   ...
   * Skapa ett `System.ServiceModel.BasicHttpBinding`-objekt genom att hämta värdet för fältet `FormDataIntegrationClient.Endpoint.Binding`. Sänd returvärdet till `BasicHttpBinding`.
...
1. Importera data till PDF-formuläret.
   ...
   * Det `BLOB`-objekt som lagrar formulärdata.
...
1. Spara PDF-formuläret som en PDF-fil.
   ...
   * Skapa en bytearray som lagrar datainnehållet i `BLOB`-objektet som returnerades av metoden `importData`. Fyll i bytearrayen genom att hämta värdet för `BLOB`-objektets `MTOM`-fält.
...
1. Exportera data från PDF-formuläret.
   ...
   Exportera formulärdata genom att anropa `FormDataIntegrationClient`-objektets `exportData`-metod och skicka `com.adobe.idp.Document`-objektet som lagrar PDF-formuläret. Den här metoden returnerar ett `com.adobe.idp.Document`-objekt som lagrar formulärdata som ett XML-schema.

1. Spara PDF-formuläret som en PDF-fil.

   * Skapa ett `java.io.File`-objekt och kontrollera att filtillägget är XML.
   * Anropa `Document`-objektets `copyToFile`-metod för att kopiera innehållet i `Document`-objektet till filen (kontrollera att du använder `Document`-objektet som returnerades av metoden `exportData`).
...
### Exportera formulärdata med webbtjänstens API {#export-form-data-using-the-web-service-api}
...
1. Referera till ett PDF-formulär.
   ...
   * Skapa ett `BLOB`-objekt med hjälp av dess konstruktor. Det här `BLOB`-objektet används för att lagra PDF-formuläret som data exporteras från.
   * Skapa ett `System.IO.FileStream`-objekt genom att anropa dess konstruktor. Skicka ett strängvärde som anger var PDF-formuläret finns och i vilket läge filen ska öppnas.
   * Skapa en bytearray som lagrar innehållet i `System.IO.FileStream`-objektet. Du kan bestämma storleken på bytearrayen genom att hämta `System.IO.FileStream`-objektets `Length`-egenskap.
...
1. Exportera data från PDF-formuläret.
   ...
   Importera data till PDF-formulär genom att anropa `FormDataIntegrationClient`-objektets `exportData`-metod och skicka `BLOB`-objektet som lagrar PDF-formuläret. Den här metoden returnerar ett `BLOB`-objekt som lagrar formulärdata som ett XML-schema.
...
1. Spara PDF-formuläret som en PDF-fil.
   ...
   * Skapa en bytearray som lagrar datainnehållet i `BLOB`-objektet som returnerades av metoden `exportData`. Fyll i bytearrayen genom att hämta värdet för `BLOB`-objektets `MTOM`-fält.
   * Skapa ett `System.IO.BinaryWriter`-objekt genom att anropa dess konstruktor och skicka `System.IO.FileStream`-objektet.
