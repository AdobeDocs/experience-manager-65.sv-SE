...
>[!NOTE]
>
>Om du vill återge ett formulär som innehåller användningsrättigheter måste du använda en XDP-fil som indata, inte en PDF-fil. Om du använder en PDF-fil som indata återges formuläret fortfarande; Det kommer dock inte att vara ett rättighetsaktiverat formulär.
...
1. Ange körningsalternativ för användningsrättigheter
   ...
   * Ställ in varje användningsbehörighet genom att anropa motsvarande metod som tillhör `ReaderExtensionSpec` -objekt. Du kan dock bara ange en användningsbehörighet om de referenser du refererar till tillåter det. Det innebär att du inte kan ange en användningsbehörighet om du inte kan ange den i autentiseringsuppgifterna. Till exempel. för att ställa in användarbehörighet som gör att användaren kan fylla i formulärfält och spara formuläret, anropa `ReaderExtensionSpec` objektets `setReFillIn` metod och skicka `true`.
