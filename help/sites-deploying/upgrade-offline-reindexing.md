...
# Använda omindexering offline för att minska driftavbrott under en uppgradering {#offline-reindexing-to-reduce-downtime-during-upgrades}
...
I det här avsnittet beskrivs hur du använder Oak-run-verktyget för att indexera om databasen **innan** utför uppgraderingen, vilket minskar tiden för driftavbrott under den faktiska uppgraderingen. Stegen som visas kan användas i [Lucene](https://jackrabbit.apache.org/oak/docs/query/lucene.html)-index för version AEM 6.4 och senare.
...
Det problem som de flesta kunder står inför under en uppgradering är att tiden för driftstopp minskar. Lösningen är att **hoppa över omindexeringsaktiviteten under uppgraderingen.** Detta kan du göra genom att skapa de nya versionerna **innan** för att utföra uppgraderingen och sedan importera dem under uppgraderingen.

## Inflygning {#approach}

![offline-reindexing-upgrade-text-extraction](assets/offline-reindexing-upgrade-process.png)

Tanken är att skapa indexet före uppgraderingen, mot indexdefinitionerna för AEM med verktyget [Oak-run](/help/sites-deploying/indexing-via-the-oak-run-jar.md). I bilden ovan visas omindexeringsmetoden offline.
...
Där `nodestore path` är `mongo_ur` eller `crx-quickstart/repository/segmentstore/`
...
**3. Kör textextraheringsprocessen med kodbiblioteket för binärfilerna som saknas i ovanstående steg**
...
Mer information om textraheringsprocessen finns i [dokumentationen för Oak-run](https://jackrabbit.apache.org/oak/docs/query/pre-extract-text.html).
...
**1. Generera indexdefinitioner för Oak Lucene för AEM**
...
Kör det här kommandot om du vill dumpa indexdefinitionen från **källinstansen** AEM:

>[!NOTE]
>
>Mer information om dumpningsindexdefinitioner finns i [ekdokumentationen](https://jackrabbit.apache.org/oak/docs/query/oak-run-indexing.html#async-index-data).

```
java -jar oak-run.jar index --fds-path <datastore path> <nodestore path> --index-definitions
```

Där `datastore path` och `nodestore path` är från **källinstansen** AEM.

Generera sedan indexdefinitioner från **mål**-AEM med målversionens Granite-databasket.
...
>[!NOTE]
>
> Processen för att skapa indexdefinitioner ovan stöds endast från och med versionen `oak-run-1.12.0`. Målsättningen görs med Granite-databaskaketet `com.adobe.granite.repository-x.x.xx.jar`.
...
Skapa en kontrollpunkt i produktionsinstansen **source** AEM med lång livstid. Detta bör göras innan databasen klonas.

Via JMX-konsolen på `http://serveraddress:serverport/system/console/jmx` går du till `CheckpointMBean` och skapar en kontrollpunkt med lång livslängd (till exempel 200 dagar). Anropa därför `CheckpointMBean#createCheckpoint` med `17280000000` som argument för livslängdens längd i millisekunder.
...
Mer information finns i [skapande av kontrollpunkter](https://jackrabbit.apache.org/oak/docs/query/oak-run-indexing.html#out-of-band-create-checkpoint) i dokumentationen för Oak.

**Utför offlineindexering för de genererade indexdefinitionerna**

Omindexering av Lucene kan göras offline med ekkörning. Den här processen skapar indexdata på disken under `indexing-result/indexes`. Det skriver **inte** till databasen och behöver därför inte stoppa den AEM som körs. Det skapade textarkivet matas in i den här processen:
...
Användning av parametern `--doc-traversal-mode` är praktiskt med MongoMK-installationer eftersom den avsevärt förbättrar indexeringstiden genom att mellanlagra databasinnehåll i en lokal platt fil. Det kräver dock ytterligare diskutrymme som är dubbelt så stort som databasens storlek.
...
Ytterligare teknisk information finns i [ekveringsdokumentationen för indexering](https://jackrabbit.apache.org/oak/docs/query/oak-run-indexing.html).

### Importerar index {#importing-indexes}

Med AEM 6.4 och senare versioner har AEM den inbyggda möjligheten att importera index från disk i startsekvenser. Mappen `<repository>/indexing-result/indexes` bevakas med avseende på om det finns indexdata under start. Du kan kopiera det redan skapade indexet till ovanstående plats under [uppgraderingsprocessen](in-place-upgrade.md#performing-the-upgrade) innan du börjar med den nya versionen av **mål** AEM jar. AEM importerar den till databasen och tar bort motsvarande kontrollpunkt från systemet. Därför undviks helt omindexering.

## Ytterligare tips och felsökning {#troubleshooting}
...
### Förbered en körning av en Runbook och en utvärderingsversion {#prepare-a-runbook-and-trial-run}
...
Du kan använda dokumentbläddringsläget genom att lägga till kommandoradsparametern `—doc-traversal` till kommandot ekörning för offlineindexering. I det här läget mellanlagras en kopia av hela databasen på den lokala disken som en platt fil och används för att köra indexeringen.
