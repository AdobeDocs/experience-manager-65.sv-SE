...
Nya versioner av AEM gör förändringar i Oak indexdefinitioner allt eftersom funktionsuppsättningen utökas. Ändringar av Oak-index tvingar till omindexering när AEM uppgraderas. Omindexering är dyrt för resursinstallationer eftersom text i resurser (till exempel text i pdf-fil) extraheras och indexeras. Med MongoMK-databaser bevaras data över nätverket vilket ytterligare ökar tiden för omindexering.
...
**1. Generera Oak Lucene-indexdefinitioner för AEM**
...
Kör det här kommandot om du vill dumpa indexdefinitionen från AEM **source**:
...
Var `datastore path` och `nodestore path` kommer från AEM **source**.

Generera sedan indexdefinitioner från AEM **target** med målversionens Granite-databaspaket.
